...
int main() {
  ...
  while (1) {
    ...
    FD_SET(self_pipe[READ], &read_fds);
    //close(self_pipe[READ]);
    
    if (started) write(self_pipe[WRITE], "", sizeof(""));

    //select will block until either fd is ready
    //select(listen_socket + 1, &read_fds, NULL, NULL, NULL); //blocking here
    if (started) select(self_pipe[READ] + 1, &read_fds, NULL, NULL, NULL); //blocking here
    else select(listen_socket + 1, &read_fds, NULL, NULL, NULL); //blocking here
	...
	***/
	...
	for (pipes = 0; pipes < subserver_count; pipes ++) {
	  close(ss_to_server[2 * pipes + WRITE]);
	  read(ss_to_server[2 * pipes + READ], &buffer, sizeof(buffer));
	  printf("Reading from buffer... %s %s\n", buffer, RESET);
	  read(ss_to_server[2 * pipes + READ], &buffer, sizeof(buffer));
	  printf("Reading from buffer... %s %s\n", buffer, RESET);
	}
      }
